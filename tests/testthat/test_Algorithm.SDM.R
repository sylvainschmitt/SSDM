test_that("modelling works", {
  ...
  model = evaluate(model, cv = "holdout", cv.param = c(0.7, 2), final.fit.data = 'all', thresh = 1001,
                   metric = "SES", Env)
  expect_equal(dim(model@evaluation), c(1, 8))
  expect_equal(dim(model@data), c(1057, 7))
  expect_true(is.logical(model@data$train))
  model = project(model, Env)
  expect_equal(all(is.na(values(model@projection))), FALSE)
  model = evaluate.axes(model, cv.param = c(0.7, 2), final.fit.data = 'all', thresh = 1001, metric = "SES",
