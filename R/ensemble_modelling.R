...
#' ESDM <- ensemble_modelling(c("CTA", "MARS"), Occurrences, Env,
#'   rep = 1,
...
ensemble_modelling <- function(algorithms,
  ...
  # Check arguments
  .checkargs(
    Xcol = Xcol, Ycol = Ycol, Pcol = Pcol, rep = rep, name = name, save = save,
  ...
  available.algo <- c(
    "GLM", "GAM", "MARS", "GBM", "CTA", "RF", "MAXENT",
    "ANN", "SVM"
  )
  ...
  if (verbose) {
    cat(sprintf(
      "#### Algorithms models creation for %s ##### %s \n\n",
  ...
  if (cores > 0) {
    if ((detectCores() - 1) < cores) {
      cores <- detectCores() - 1
      warning(paste(
        "It seems you attributed more cores than your CPU has! Automatic reduction to",
        cores, "cores."
      ))
    }
    ...
    if (parmode == "replicates") {
      for (i in seq_len(length(algorithms))) {
        ...
        modelrep <- foreach::foreach(iterators::icount(rep),
          ...
          model <- try(modelling(algorithms[i], Occurrences, Env,
            Xcol = Xcol,
          ...
          if (inherits(model, "try-error")) {
            ...
            if (!isFALSE(tmp) & !is.null(model)) {
              model@projection <- writeRaster(model@projection, paste0(
                tmppath,
                "/proba",
                model.name,
                Sys.getpid(),
              ...
              model@binary <- writeRaster(model@binary, paste0(
                tmppath,
                "/bin",
                model.name,
                Sys.getpid(),
        ...
        if (GUI) {
          incProgress(1 / (length(algorithms) + 1), detail = paste(
            algorithms[i],
            "SDM built"
          ))
        }
    ...
    if (parmode == "algorithms") {
      ...
      models <- foreach::foreach(
        i = 1:length(algorithms), .packages = c("raster", "SSDM"),
        .verbose = verbose
      ) %dopar% {
        modelrep <- list()
        for (j in 1:rep) {
  ...
  if (verbose) {
    cat(sprintf(
      "#### Ensemble modelling with algorithms models for %s ##### %s \n\n",
      spname, format(Sys.time(), "%Y-%m-%d %T")
    ))
  }
  algo <- list()
