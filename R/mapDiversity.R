...
#' @rdname mapDiversity
#' @export
setGeneric("mapDiversity", function(obj, ...) {
  return(standardGeneric("mapDiversity"))
})
...
setMethod("mapDiversity", "Stacked.SDM", function(obj, method, rep.B = 1000,
  ...
  for (i in seq_len(length(obj@esdms))) {
    if (!inherits(obj@esdms[[i]]@projection, "RasterLayer")) {
      if (verbose) {
        cat(
          "Error", obj@esdms[[i]]@name, "is not a raster but a",
  ...
  if (method == "MaximumLikelihood") {
    ...
    corr.sr <- rowSums(apply(pred.prob, 2, FUN = function(x) {
      invlogit(logit(x) + adj.par$par[1] * obs.sr + adj.par$par[2])
    }))
  ...
  if (method == "PRR.pSSDM") {
    ...
    diversity.map <- mapDiversity(obj,
      method = "pSSDM",
      verbose = FALSE
    )$diversity.map
    esdms <- .PRR(obj, diversity.map)
...
.richness <- function(obj) {
  ...
  for (i in seq_len(length(obj@esdms))) {
    Richness <- Richness + rasterize(
      ...
      Richness,
      field = obj@esdms[[i]]@data$Presence,
      background = 0
    )
  }
...
.PRR <- function(obj, Richness) {
  ...
  probabilities <- lapply(
    lapply(obj@esdms, FUN = slot, name = "projection"),
    values
  )
  ...
  binaries <- lapply(
    lapply(obj@esdms, FUN = slot, name = "binary"),
    values
  )
  ...
  binaries <- mapply(
    function(rich, probability, binary) {
      if (!is.na(rich)) {
