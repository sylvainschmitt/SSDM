...
.optim.thresh <- function(obs, pred, threshold = 101) {
  ...
  if (length(threshold) == 1 & threshold[1] > 1) {
    thresholds <- seq(0, 1, length = threshold)
  } else {
    stop("inappropriate threshold value. See help file")
  }
  ...
  n <- length(obs)
  if (length(which(obs %in% c(0, 1))) != n) stop("observed values must be 0 or 1") # ensure observed are values 0 or 1
  ...
  if (length(t.thresh) > 1) {
    out$"10.percent.omission" <- range(t.thresh)
  } else {
    out$"10.percent.omission" <- t.thresh
  }

  # sensitivity = specificity
  t.thresh <- ac$threshold[which(rank(abs(ac$sensitivity - ac$specificity), ties.method = "min") == 1)]
  if (length(t.thresh) > 1) {
    out$"sensitivity=specificity" <- range(t.thresh)
  } else {
    out$"sensitivity=specificity" <- t.thresh
  }

  # max sensitivity + specificity
  t.thresh <- ac$threshold[which(rank(-(ac$sensitivity + ac$specificity), ties.method = "min") == 1)]
  if (length(t.thresh) > 1) {
    out$"max.sensitivity+specificity" <- range(t.thresh)
  } else {
    out$"max.sensitivity+specificity" <- t.thresh
  }

  # max Kappa
  t.thresh <- ac$threshold[which(rank(-(ac$Kappa), ties.method = "min") == 1)]
  if (length(t.thresh) > 1) {
    out$maxKappa <- range(t.thresh)
  } else {
    out$maxKappa <- t.thresh
  }

  # Max prop correct
  t.thresh <- ac$threshold[which(rank(-(ac$prop.correct), ties.method = "min") == 1)]
  if (length(t.thresh) > 1) {
    out$max.prop.correct <- range(t.thresh)
  } else {
    out$max.prop.correct <- t.thresh
  }

  # min ROC plot distance
  t.thresh <- ac$threshold[which(rank((1 - ac$sensitivity)^2 + (ac$specificity - 1)^2, ties.method = "min") == 1)]
  if (length(t.thresh) > 1) {
    out$min.ROC.plot.distance <- range(t.thresh)
  } else {
    out$min.ROC.plot.distance <- t.thresh
  }
