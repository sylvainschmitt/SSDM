...
.accuracy <- function(obs, pred, threshold = 0.5) {
  ...
  if (length(threshold) == 1 & threshold[1] <= 1 & threshold[1] >= 0) {
    ...
    stop("inappropriate threshold values used as input. See help file.")
  }

  # cycle through each of the helpfiles
  out <- data.frame(
    threshold = as.double(thresholds), AUC = NA, omission.rate = NA, sensitivity = NA,
  ...
  for (ii in 1:length(thresholds)) {
    ...
    bin.pred <- pred
    if (threshold == 0) {
      bin.pred[which(bin.pred > threshold)] <- 1
      bin.pred[which(bin.pred <= threshold)] <- 0
    } else {
      bin.pred[which(bin.pred >= threshold)] <- 1
      bin.pred[which(bin.pred < threshold)] <- 0
    }
...
.auc <- function(obs, pred) {
  ...
  if (n1 == 0 || n1 == n) {
    return(NaN)
  } # if all observed 1's or 0's return NaN
...
.omission <- function(mat) {
  # input checks
  if (inherits(mat, "confusion.matrix")) stop("mat must be of class confusion.matrix")
  # return the value
