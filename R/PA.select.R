...
setMethod('PA.select', "Algorithm.SDM", function(obj, Env, PA = NULL, verbose = TRUE) {
  ...
  if(PA$strat=='geobuffer'){
    ...
    buf_pts <- geobuffer::geobuffer_pts(xy=cbind(obj@data$X,obj@data$Y),dist_m = dist_m,crs=as.character(crs(Env[[1]])),output="sf")
    buf_ras <- fasterize::fasterize(sf=buf_pts, raster=Env[[1]])
