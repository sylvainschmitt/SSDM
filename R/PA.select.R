...
setGeneric("PA.select", function(obj, Env, ...) {
  return(standardGeneric("PA.select"))
})
...
setMethod("PA.select", "Algorithm.SDM", function(obj, Env, PA = NULL, verbose = TRUE) {
  ...
  if (PA$strat == "geobuffer") {
    ...
    buf_pts <- geobuffer::geobuffer_pts(
      xy = cbind(obj@data$X, obj@data$Y),
      dist_m = dist_m, crs = as.character(crs(Env[[1]])),
      output = "sf"
    )
    buf_ras <- fasterize::fasterize(
      sf = buf_pts,
      raster = Env[[1]]
    )
  ...
  if (PA$nb > length(mask_inds)) {
    ...
    if (verbose) {
      cat("Number of requested pseudo-absences exceeds number of cells, lowered to number of cells \n")
    }
  }
