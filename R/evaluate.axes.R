...
#' @import methods
NULL
setGeneric("evaluate.axes", function(obj, cv = "holdout", cv.param = c(0.7, 2), final.fit.data = "all", bin.thresh = "SES", metric = NULL, thresh = 1001, axes.metric = "Pearson", Env, ...) {
  return(standardGeneric("evaluate.axes"))
})
    ...
    for (i in 4:(length(obj@data) - 1)) {
      ...
      if (axes.metric != "Pearson") {
        ...
        predicted.values <- predict(
          get_model(obj.axes, Env = Env[[-(i - 3)]], ...),
