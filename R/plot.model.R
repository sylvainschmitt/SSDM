...
setMethod("plot", "Stacked.SDM", function(x, y, ...) {
  choices <- list()
  for (i in seq_len(length(x@esdms))) {
    choices[[i]] <- strsplit(x@esdms[[i]]@name, ".Ensemble.SDM", fixed = TRUE)[[1]][1]
  }
  if (inherits(x@esdms[[1]], "Algorithm.SDM")) {
    full <- FALSE
  } else {
    full <- TRUE
  }
  ...
  ui <- dashboardPage(
    ...
    dashboardSidebar(
      sidebarMenu(
        ...
        selectInput("esdmchoice", "Species:", choices,
  ...
  server <- function(input, output) {
    # Main page beginning #
    # Maps
    output$diversity.title <- renderText({
      paste0("<b>Mean species richness error: ", round(x@evaluation["mean", "species.richness.error"], 3), "<br>")
    })
    ...
    output$varimp.barplot <- renderPlot({
      ...
      bar <- barplot(varimp$Axes.evaluation,
        names.arg = abbreviate(row.names(varimp)),
    ...
    output$varimplegend <- renderTable({
      data.frame("Abbreviation" = abbreviate(names(x@variable.importance)), "Variable" = names(x@variable.importance))
    })
    # Parameters
    output$summary <- renderTable({
      ...
      row.names(summary) <- c(
        "Occurrences type", "Final number of species", "Original algorithms", "Number of repetitions",
    ...
    output$probability.title <- renderText({
      paste0(
        "<b>AUC :", round(x@esdms[[which(choices == input$esdmchoice)]]@evaluation$AUC, 3),
    ...
    output$esdm.evaluation.barplot <- renderPlot({
      for (i in seq_len(length(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)))) {
        row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i] <- strsplit(as.character(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i]), ".SDM", fixed = TRUE)[[1]][1]
      }
      for (i in seq_len(length(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)))) {
        row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i] <- tail(strsplit(as.character(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i]), ".", fixed = TRUE)[[1]], n = 1)
      }
    ...
    output$esdm.evaluation.table <- renderTable({
      for (i in seq_len(length(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)))) {
        row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i] <- strsplit(as.character(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i]), ".SDM")[[1]][1]
      }
      for (i in seq_len(length(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)))) {
        row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i] <- tail(strsplit(as.character(row.names(x@esdms[[which(choices == input$esdmchoice)]]@algorithm.evaluation)[i]), ".", fixed = TRUE)[[1]], n = 1)
      }
    ...
    output$esdm.varimp.table <- renderTable({
      x@esdms[[which(choices == input$esdmchoice)]]@variable.importance
    })
    output$varimplegend <- renderTable({
      data.frame("Abbreviation" = abbreviate(names(x@esdms[[which(choices == input$esdmchoice)]]@variable.importance)), "Variable" = names(x@esdms[[which(choices == input$esdmchoice)]]@variable.importance))
    })
    # Parameters
    output$esdm.summary <- renderTable({
      ...
      names(summary) <- "Summary"
      row.names(summary) <- c(
        "Occurrences type", "Occurrences number", "Final number of species", "Original algorithms", "Number of repetitions",
        "Pseudo-absences selection", "Cross validation method", "Cross validation parameters"
      )
      algo.info <- character()
      ...
      if (x@esdms[[which(choices == input$esdmchoice)]]@parameters$PA) {
        PA <- "default"
      }
      ...
      if (x@esdms[[which(choices == input$esdmchoice)]]@parameters$cv == "LOO") {
        cv.param <- "None"
      }
      if (x@esdms[[which(choices == input$esdmchoice)]]@parameters$cv == "holdout") {
        cv.param <- paste(
          "fraction =",
    ...
    output$esdm.binary.info <- renderText({
      ...
      text <- paste(
        "Binary map realized by", x@esdms[[which(choices == input$esdmchoice)]]@parameters$metric,
    ...
    output$esdm.evaluation.info <- renderText({
      ...
      if (x@esdms[[which(choices == input$esdmchoice)]]@parameters$weight) {
        evaluation.info <- paste(evaluation.info, ", and then weighted with the previous metrics means")
      }
      evaluation.info
    })
...
setMethod("plot", "SDM", function(x, y, ...) {
  ...
  ui <- dashboardPage(
    dashboardHeader(title = x@name, titleWidth = 450),
    dashboardSidebar(disable = TRUE), dashboardBody(fluidRow(tabBox(
      title = "Maps",
      ...
      fluidRow(tabBox(title = "Model evaluation", tabPanel(plotOutput("evaluation.barplot"),
        textOutput("evaluation.info"),
        title = "Barplot"
      ), tabPanel(tableOutput("evaluation.table"),
        title = "Table"
      )), if (length(x@algorithm.correlation) > 0) {
        tabBox(title = "Algorithms correlation", tabPanel(plotOutput("algo.corr.heatmap"),
          title = "Heatmap"
        ), tabPanel(tableOutput("algo.corr.table"),
          title = "Table"
        ))
      })
    })
  ...
  server <- function(input, output) {
    ...
    output$probability.title <- renderText({
      paste0(
        "<b>AUC :", round(x@evaluation$AUC, 3),
    ...
    output$niche <- leaflet::renderLeaflet({
      niche.map <- reclassify(x@projection, c(
        -Inf, x@evaluation$threshold,
    ...
    if (full) {
      ...
      output$evaluation.barplot <- renderPlot({
        ...
        barplot(table,
          col = rainbow(length(metrics)), names.arg = row.names(evaluation),
          beside = TRUE
        )
    ...
    output$varimp.barplot <- renderPlot({
      varimp <- as.data.frame(t(x@variable.importance))
      names(varimp) <- "Axes.evaluation"
      barplot(varimp$Axes.evaluation,
        names.arg = abbreviate(row.names(varimp)),
        las = 2
      )
    })
    ...
    output$varimplegend <- renderTable({
      data.frame(
        Abbreviation = abbreviate(names(x@variable.importance)),
    ...
    output$summary <- renderTable({
      ...
      names(summary) <- "Summary"
      row.names(summary) <- c(
        "Occurrences type", "Pseudo-absences selection",
      ...
      if (x@parameters$cv == "holdout") {
        cv.param <- paste("fraction =", strsplit(x@parameters$cv.param,
          "|",
          fixed = TRUE
        )[[1]][2], "rep =", strsplit(x@parameters$cv.param,
          "|",
          fixed = TRUE
        )[[1]][3])
      }
      ...
      if (!is.null(x@parameters$algorithms)) {
        ...
        summary <- rbind(summary, data.frame(Summary = c(
          algo.info,
          x@parameters$rep
        ), row.names = c(
          "Original algorithms",
    ...
    output$varimp.info <- renderText({
      ...
      for (i in seq_len(length(x@parameters$axes.metric))) {
        if (i == 1) {
          ...
          varimp.info <- paste(
            varimp.info, "and", x@parameters$axes.metric[i],
            "."
          )
        } else {
          varimp.info <- paste(varimp.info, ",", x@parameters$axes.metric[i])
      ...
      for (i in seq_len(length(strsplit(x@parameters$ensemble.metric,
        ...
        if (i == 1) {
          evaluation.info <- paste(evaluation.info, strsplit(x@parameters$ensemble.metric,
            ".",
            fixed = TRUE
          )[[1]][-1][i], "(>", strsplit(x@parameters$ensemble.thresh,
            "|",
            fixed = TRUE
          )[[1]][-1][i], ")")
        } else if (i == length(x@parameters$axes.metric) && i != 1) {
