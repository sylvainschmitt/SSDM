...
#' \dontrun{
#' load_occ(
#'   path = system.file("extdata", package = "SSDM"), Env,
...
load_occ <- function(path = getwd(), Env, file = NULL, ..., Xcol = "Longitude",
  ...
  # Check arguments
  .checkargs(
    path = path, file = file, Xcol = Xcol, Ycol = Ycol, Spcol = Spcol,
  ...
  if (!is.null(Spcol)) {
    if (!inherits(
      Occurrences[, which(names(Occurrences) == Spcol)],
      "factor"
    )) {
  ...
  if (!inherits(Occurrences[, which(names(Occurrences) == Xcol)], "numeric")) {
    if (inherits(
      Occurrences[, which(names(Occurrences) == Xcol)],
      "factor"
    )) {
  ...
  for (i in seq_len(length(levels(Occurrences[, which(names(Occurrences) ==
    ...
    if (GeoRes) {
      ...
      SpOccurrences <- subset(Occurrences, Occurrences[which(names(Occurrences) ==
        ...
        Spcol)]))[i])
      thin.result <- thin(SpOccurrences,
        long.col = Xcol, lat.col = Ycol,
