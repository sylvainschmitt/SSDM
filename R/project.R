...
setMethod("project", "Algorithm.SDM", function(obj, Env, output.format = "model", ...) {
  ...
  model <- get_model(obj, ...)
  if (all(names(Env) %in% colnames(obj@data)[-c(1:3)]) == FALSE) {
    stop("Environmental layer names do not match the variables used for model training")
  }
  ...
  if (all(obj@data$Presence %in% c(0, 1))) { # MEMs can't produce binary
    obj@binary <- reclassify(proj, c(
      -Inf, obj@evaluation$threshold, 0,
...
setMethod("project", "Ensemble.SDM", function(obj, Env, uncertainty = TRUE, output.format = "model", SDM.projections = FALSE, cores = 0, minimal.memory = FALSE, tmp = FALSE, ...) {
  ...
  if (cores > 0 && requireNamespace("parallel", quietly = TRUE)) {
    ...
    if (minimal.memory) {
      ...
      doParallel::registerDoParallel(cl)
      proj <- foreach(models = itertools::isplitVector(models, chunks = cores), .packages = c("raster", "SSDM", "itertools"), .verbose = F) %dopar% lapply(models, FUN = function(x) {
        project(x, Env)
      })
    ...
    # sequential
    proj <- lapply(models, FUN = function(x) {
      project(x, Env)
    })
  }
  ...
  if (output.format == "model") {
    ...
    if (uncertainty) {
      projls <- c(projls, uncertainty = sum.algo.ensemble@uncertainty)
    }
    if (SDM.projections) {
      projls <- c(projls, list(sdms = lapply(sum.algo.ensemble@sdms, function(x) list(projection = x@projection, binary = x@binary))))
    }
    return(projls)
  }
...
setMethod("project", "Stacked.SDM", function(obj, Env, method = NULL, uncertainty = TRUE, output.format = "model", SDM.projections = FALSE, cores = 0, minimal.memory = FALSE, tmp = FALSE, ...) {
  ...
  # stack ESDMs
  if (is.null(method)) {
    method <- obj@parameters[, which(names(obj@parameters) == "method")]
  }
  ...
  if (output.format == "model") {
    ...
    if (SDM.projections) {
      projls$esdms <- c(projls$esdms, list(sdms = lapply(ensemble.stack@esdms, function(x) lapply(x@sdms, function(y) list(projection = y@projection, binary = y@binary)))))
    }
    return(projls)
  }
})
